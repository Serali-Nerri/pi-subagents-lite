...
function makeTui(prefixCount = 1): any {
	...
	return {
		...
		originalFooter,
		children: [
			...Array.from({ length: prefixCount }, (_, index) =>
				makeContainer(`header ${index}`),
			),
			originalChat,
			originalPending,
			originalStatus,
...
function makePi084Tui(): any {
	const legacy = makeTui(2);
	const documentContainer = {
		children: [legacy.children[0], legacy.children[1], legacy.originalChat],
		render: () => ["document"],
		invalidate: vi.fn(),
	};
	const footerContainer = {
		children: [legacy.originalFooter],
		render: () => legacy.originalFooter.render(),
		invalidate: vi.fn(),
	};

	legacy.pendingIndex = 1;
	legacy.statusIndex = 2;
	legacy.editorIndex = 4;
	legacy.belowIndex = 5;
	legacy.footerIndex = 6;
	legacy.documentContainer = documentContainer;
	legacy.footerContainer = footerContainer;
	legacy.children = [
		documentContainer,
		legacy.originalPending,
...
describe("AgentNavigator", () => {
	...
	it("routes ordinary editor submits before Pi can queue them on Main", () => {
		...
		expect(ui.baseEditor.addToHistory).toHaveBeenCalledWith(
			"continue the child",
		);
	...
	it("reconciles a footer appended by another extension on the child screen", () => {
		...
		tui.originalFooter.render = () => {
			throw new Error("disposed footer rendered");
		};
	...
	it("supports Pi 0.84 nested document and footer containers", () => {
		const record = makeRecord();
		const ui = makeUI({ value: "" });
		navigator = new AgentNavigator(makeManager([record]));
		navigator.setUICtx(ui.ctx as any);
		navigator.ensureTimer();
		const tui = makePi084Tui();
		mountSelector(ui, tui);
		...
		expect(navigator.selectedId()).toBe(record.id);
		expect(tui.documentContainer.children[2]).not.toBe(tui.originalChat);
		expect(tui.children[tui.pendingIndex]).not.toBe(tui.originalPending);
		expect(tui.children[tui.statusIndex]).not.toBe(tui.originalStatus);
		expect(tui.footerContainer.children[0]).not.toBe(tui.originalFooter);
		expect(ui.ctx.notify).not.toHaveBeenCalledWith(
			"Subagent screen switching is unavailable: unsupported Pi TUI layout",
			"warning",
		);
